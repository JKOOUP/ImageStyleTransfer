...
class StyleTransferProcessor:
    ...
    def configure(self,
        ...
        logger.info("StyleTransferProcessor was successfully configured.")
        return self
    ...
    async def get_current_image(self) -> Image:
        ...
        logger.info("Get current style transfer result.")
    ...
    async def transfer_style(self) -> Image:
        logger.info("Started style transfer process.")
        ...
        for iteration_idx in range(self._num_iteration):
            ...
            if (iteration_idx + 1) % (self._num_iteration // 10) == 0:
                logger.info(f"Completed {100 * (iteration_idx + 1) / self._num_iteration:.2f}%")

        result: Image = await self.get_current_image()
        self._transfer_status = 0
        logger.info("Ended style transfer process.")
        return result
